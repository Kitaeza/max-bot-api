# max-bot-api v0.1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `max-bot-api` v0.1 — an async Pydantic-modeled Python client for the Max Bot API — to PyPI with full CI, MIT license, and Claude-collaboration scaffolding.
...
**Working directory:** `/home/kitaeza/github/max-bot-api`. This is a greenfield repo with only `LICENSE` and the design doc committed. Work directly on `main`; no worktree needed for greenfield.
...
[project.urls]
Homepage = "https://github.com/Kitaeza/max-bot-api"
Repository = "https://github.com/Kitaeza/max-bot-api"
Documentation = "https://github.com/Kitaeza/max-bot-api/blob/main/README.md"
Changelog = "https://github.com/Kitaeza/max-bot-api/blob/main/CHANGELOG.md"
Issues = "https://github.com/Kitaeza/max-bot-api/issues"
...
```bash
cd /home/kitaeza/github/max-bot-api
uv sync
...
jobs:
  build-and-publish:
    ...
    environment:
      name: pypi
      url: https://pypi.org/p/max-bot-api
    steps:
      - uses: actions/checkout@v4
...
3. Fill in:
   - PyPI Project Name: `max-bot-api`
   - Owner: `Kitaeza`
   - Repository name: `max-bot-api`
...
1. Go to <https://github.com/Kitaeza/max-bot-api/settings/environments>.
...
This avoids the awkward case where someone else grabs `max-bot-api` between
...
curl -s https://pypi.org/pypi/max-bot-api/json | python -c "import json, sys; d = json.load(sys.stdin); print(d['info']['version'])"

# Test install in a throwaway env
mkdir -p /tmp/maxbot-smoketest && cd /tmp/maxbot-smoketest
uv venv && source .venv/bin/activate
uv pip install max-bot-api
...
- Modify: `pyproject.toml` (add `max-bot-api>=0.1` dependency)
...
This task is **out of scope for the max-bot-api repo** — it lives in Postbayan and is the integration step. It's listed here so the plan ends with a complete loop ("library shipped + consumer wired"), but execution happens in the other repo and follows Postbayan's own brainstorming/planning conventions. If you're executing this plan inside max-bot-api, **stop here** and switch repos for Task 17.
